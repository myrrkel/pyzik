...
class radioManager():
    ...
    def searchDirbleRadios(self, search):
        ...
        search = urllib.parse.quote_plus(search)
        try:
            r = requests.post("http://api.dirble.com/v2/search?token=" + dirbleAPIKey, data={'query': search},
                              timeout=2)
    ...
    def searchTuneinRadios(self, search):
        ...
        try:
            r = requests.post(
                "https://api.radiotime.com/profiles?query=" + search + "&filter=s!&fullTextSearch=true&limit=20&formats=mp3,aac,ogg")
            tradios = json2obj(r.text)
        except requests.exceptions.HTTPError as err:
            print(err)
