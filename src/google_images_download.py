...
class googleimagesdownload:
    ...
    def download_page(self,url):
        try:
            ...
            # respData = resp.content.decode('utf-8', 'ignore')
    ...
    def build_url_parameters(self,arguments):
        ...
        logger.debug('BUILT URL: %s', built_url)
        return built_url
    ...
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        items = []
        e1 = 0
        while count < limit+1 and (e1 > 0 or count == 1):
            b1 = object_raw.find('[1,[0,', e1)
            e1 = object_raw.find('[1,[0,', b1 + 1)

            img_raw_data = str(object_raw[b1:e1-3])
            logger.debug("img_raw_data: %s", img_raw_data)
            txt = '{"datas": ['+img_raw_data+']]}'
                ...
                if isinstance(line, list):
                    for el in line:
                        ...
                        if isinstance(el, list) and len(el) == 3:
                            if not res.get('thumbnail', False):
                                res['thumbnail'] = el
                                res["image_thumbnail_url"] = el[0]
                            else:
                                res['image'] = el
                                res["image_link"] = el[0]
                                res["image_height"] = el[1]
                                res["image_width"] = el[2]
                                items.append(res)
                                count += 1
                                break
                    break
